...
export default function WatchlistPanel() {
  ...
  return (
    <div className="flex flex-col flex-1 bg-zinc-950 border border-zinc-800 rounded overflow-hidden select-none">
      ...
      <form onSubmit={onSubmit} className="p-2 border-b border-zinc-900 flex gap-1 shrink-0">
        <input
          type="text"
          maxLength={6}
          placeholder="Code (e.g. 005930)"
          value={searchSymbol}
          onChange={(e) => setSearchSymbol(e.target.value.replace(/[^0-9]/g, ''))}
          className="w-20 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-1 text-xs text-white focus:outline-none focus:border-zinc-700 font-mono"
        />
        <input
          type="text"
          placeholder="Name"
          ...
          className="flex-1 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-1 text-xs text-white focus:outline-none focus:border-zinc-700"
        />
        <button
          type="submit"
          className="px-2 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold cursor-pointer"
        >
          +
        </button>
      </form>
            ...
            {tickers.map((ticker) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={`py-2 px-2 text-right font-mono font-semibold ${changeColor}`}>
                    {ticker.change > 0 ? '+' : ''}{ticker.change}%
                  </td>
                  <td className="py-2 px-3 text-center" onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={() => handleRemoveTicker(ticker.symbol)}
                      className="p-1 hover:text-rose-400 text-zinc-600 cursor-pointer"
                      title="삭제"
                    >
