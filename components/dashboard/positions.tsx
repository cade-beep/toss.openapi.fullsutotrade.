    ...
    return (
      <div className="flex flex-col flex-1 bg-zinc-950 border border-zinc-900 rounded overflow-hidden select-none">
        <div className="flex items-center justify-between px-3 py-1.5 border-b border-zinc-900/60 shrink-0">
          <h2 className="text-[10px] uppercase font-bold tracking-wider text-zinc-500">{t('positions.title')}</h2>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <span className="text-[10px] text-zinc-550 max-w-xs mt-1 font-sans">{t('positions.apiRequired')}</span>
        </div>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[9px] uppercase tracking-wider text-zinc-500 border-b border-zinc-900/60 font-mono">
              <th className="py-1.5 px-3">{t('positions.symbol')}</th>
              <th className="py-1.5 px-2 text-right">{t('positions.qty')}</th>
              <th className="py-1.5 px-2 text-right">{t('positions.avgPrice')}</th>
              <th className="py-1.5 px-2 text-right">{t('positions.currentPrice')}</th>
              <th className="py-1.5 px-2 text-right">{t('positions.pnl')}</th>
              <th className="py-1.5 px-3 text-center">{t('positions.action')}</th>
            </tr>
          </thead>
              ...
              positions.map((pos) => {
                ...
                return (
                  <tr key={pos.symbol} className="hover:bg-zinc-900/20 transition-colors">
                    ...
                    <td className="py-1.5 px-2 text-right text-zinc-400">{formatCurrency(pos.avgBuyPrice)}</td>
                    <td className="py-1.5 px-2 text-right text-zinc-400">{formatCurrency(currentPrice)}</td>
